		...
		for(i=0; i<events; i++)
		{
			...
			else /* 已经连接的客户有发数据来 */
			{
				...
				else
				{
					...
					sqlite3_exec(sqt_db,"create table device_temp(sqt_id integer primary key,name char,value char);",NULL,NULL,NULL);
                    
                    if(unpack_server(buf,&sqt_db)<0);
                    {
                        send_ack_or_nack(&snd_ack,5);//发NACK
                    }
                    else
                        send_ack_or_nack(&snd_ack,4);//发ACK
                    memcpy(buf,&snd_ack,sizeof(snd_ack));
                    write(event_array[i].data.fd, buf, sizeof(snd_ack));
                    printf("already send ack!");
